...
class Row(BasicStorage):
    ...
    # MR: order dict format
    def as_ordered_dict(self, datetime_to_str=False, custom_types=None):
        SERIALIZABLE_TYPES = [str, int, float, bool, list, dict]
        DT_INST = (datetime.date, datetime.datetime, datetime.time)
        if PY2:
            SERIALIZABLE_TYPES += [unicode, long]
...
class BasicRows(object):
    ...
    def as_list(
        ...
        if storage_to_dict:
            ...
            # MR: default oder dict
            # items = [item.as_dict(datetime_to_str, custom_types) for item in self]
            items = [item.as_ordered_dict(datetime_to_str, custom_types) if getattr(item, 'as_ordered_dict', None) else item.as_dict(datetime_to_str, custom_types) for item in self]
        else:
