...
class DAL(with_metaclass(MetaDAL, Serializable, BasicStorage)):
    ...
    # Metaer: import one table
    def import_table_definition(
        self, path, migrate=False, fake_migrate=False, table=None, table_class=Table
    ):
        if not table:
            return None
        pattern = pjoin(path, self._uri_hash + "_*.table")
        file_pattern = pjoin(path, self._uri_hash + "_" + table +".table")
        for filename in glob.glob(file_pattern):
            ...
            try:
                ...
                self.define_table(
                    ...
                    **dict(migrate=migrate, fake_migrate=fake_migrate, table_class=table_class)
                )
                # Metaer: add _dbt
                getattr(self, name)._dbt = filename
